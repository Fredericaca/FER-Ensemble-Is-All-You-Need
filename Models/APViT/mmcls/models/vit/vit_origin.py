...
class VisionTransformerOrigin(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
                ...
                vit_pool_configs=None, last_ln=True,
    ...
    def forward_features(self, x):
        if isinstance(x, list):
            assert len(x) == 1, 'ViT Origin only accept input with on group of set, but got %s' % len(x)
            x = x[0]
        # x = x[0] + x[1]
        B = x.shape[0]
